...
async function serveCatalog(req, res, cacheKey, sql) {
  ...
  try {
    ...
    console.error(`[ERROR] al obtener ${cacheKey}:`, err.message);
...
// =============================================
app.get('/api/facultades', (req, res) => { serveCatalog(req, res, 'facultades', 'SELECT id_facultad, nombre_facultad FROM FACULTAD ORDER BY nombre_facultad ASC'); });
...
// =============================================
// RUTA PRINCIPAL: Registro de Usuarios
// =============================================
app.post('/api/auth/registro', async (req, res) => {
  ...
  try {
    ...
    const isAdminAction = (creadoPorAdmin === true || creadoPorAdmin === 'true');
    console.log(`[REGISTRO] AdminActivo: ${isAdminAction} | Carné: ${datos.carne}`);

    const salt = await bcrypt.genSalt(10);
    const contrasenaEncriptada = await bcrypt.hash(datos.password, salt);

    const requiereCambio = isAdminAction ? 1 : 0; 
    ...
    await connection.execute(sqlEmergencia, {
      carne: datos.carne,
    ...
    // 🔥 HTML DINÁMICO PARA CORREO
    const mensajePasswordHtml = isAdminAction 
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
            ...
            <div style="padding: 30px; color: #333333;">
              ...
              <p>Tu cuenta para el <strong>Sistema de Control de Parqueo</strong> ha sido creada.</p>
              ...
              <p>Ya puedes iniciar sesión en nuestro portal web.</p>
            </div>
          </div>
        `
      };
      await transporter.sendMail(mailOptions);
      console.log(`[EMAIL OK] Enviado a: ${datos.correo_electronico}`);
    } catch (errEmail) {
      console.error("[EMAIL ERROR] Falló el correo:", errEmail.message);
    }
    ...
    if (err.errorNum === 1) return res.status(400).json({ error: "Carné o correo ya registrado." });
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (!contrasenaValida) return res.status(401).json({ error: "Carné o contraseña incorrectos." });
...
// =============================================
// RUTA: Forzar Cambio de Contraseña Temporal
// =============================================
app.put('/api/auth/cambiar-password', async (req, res) => {
  ...
  try {
    ...
    const sql = `UPDATE USUARIOS SET contrasena = :contrasena, requiere_cambio_pass = 0 WHERE carne = :carne`;
    
    await connection.execute(sql, { contrasena: contrasenaEncriptada, carne });
    await connection.commit();

    console.log(`[OK] Contraseña actualizada para: ${carne}`);
...
app.get('/api/admin/usuarios', async (req, res) => {
  ...
  try {
    connection = await oracledb.getConnection(dbConfig);
    const sql = `
      SELECT u.carne, u.nombres || ' ' || u.apellidos AS nombre, u.correo_institucional AS correo, 
             r.nombre_rol AS rol, CASE WHEN u.activo = 1 THEN 'Activo' ELSE 'Inactivo' END AS estado
...
app.put('/api/admin/usuarios/:carne/estado', async (req, res) => {
  ...
  try {
    ...
    connection = await oracledb.getConnection(dbConfig);
    await connection.execute(`UPDATE USUARIOS SET activo = :nuevoEstado WHERE carne = :carne`, { nuevoEstado, carne });
    await connection.commit();
