...
class PagoStore {
  ...
  static async getAll() {
    const [rows] = await sequelize.query(`
      ...
      LEFT JOIN INFRA_DEV.CB_USUARIO_MULTA um ON p.EMU_USUARIO_MULTA = um.EMU_USUARIO_MULTA
      LEFT JOIN INFRA_DEV.CB_MULTA m ON um.MUL_MULTA = m.MUL_MULTA
      WHERE NOT (p.PAG_ESTADO = 'P' AND EXISTS (
          SELECT 1 FROM INFRA_DEV.CB_PAGO p2 
          WHERE p2.LR_CARNE = p.LR_CARNE 
            AND p2.PAG_ESTADO IN ('C', 'A') 
            AND (p2.PLN_PLAN = p.PLN_PLAN OR p2.EMU_USUARIO_MULTA = p.EMU_USUARIO_MULTA)
      ))
